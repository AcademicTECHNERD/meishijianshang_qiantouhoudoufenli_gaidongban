    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
        <el-row :gutter="20" class="slt"
          ...
          <el-form-item :label="contents.inputTitle == 1 ? '添加时间' : ''">
            <el-date-picker v-model="searchForm.tianjiashijianstart" type="date" placeholder="添加时间">
            </el-date-picker>
            至
            <el-date-picker v-model="searchForm.tianjiashijianend" type="date" placeholder="添加时间">
            </el-date-picker>
          </el-form-item>
          <el-form-item>
        ...
        <el-row class="ad"
          ...
          <el-form-item>
            <el-button
              v-if="isAuth('wodehaoyou', '新增') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1"
              type="success" icon="el-icon-plus" @click="addOrUpdateHandler()">{{ contents.btnAdAllFont == 1 ? '新增' : ''
              }}</el-button>
            <el-button
              v-if="isAuth('wodehaoyou', '新增') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2"
              type="success" @click="addOrUpdateHandler()">{{ contents.btnAdAllFont == 1 ? '新增' : '' }}<i
                class="el-icon-plus el-icon--right" /></el-button>
            <el-button v-if="isAuth('wodehaoyou', '新增') && contents.btnAdAllIcon == 0" type="success"
              @click="addOrUpdateHandler()">{{ contents.btnAdAllFont == 1 ? '新增' : '' }}</el-button>
            <el-button
              v-if="isAuth('wodehaoyou', '删除') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1 && contents.tableSelection"
              :disabled="dataListSelections.length <= 0" type="danger" icon="el-icon-delete" @click="deleteHandler()">{{
                contents.btnAdAllFont == 1 ? '删除':'' }}</el-button>
            <el-button
              v-if="isAuth('wodehaoyou', '删除') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2 && contents.tableSelection"
            ...
            <el-button v-if="isAuth('wodehaoyou', '删除') && contents.btnAdAllIcon == 0 && contents.tableSelection"
              :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">{{ contents.btnAdAllFont
                == 1?'删除':'' }}</el-button>
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
          ...
          v-if="isAuth('wodehaoyou', '查看')" :data="dataList" v-loading="dataListLoading"
          ...
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="tianjiashijian"
            header-align="center" label="添加时间">
            <template slot-scope="scope">
              {{ scope.row.tianjiashijian }}
            </template>
          </el-table-column>
          <el-table-column width="300" :align="contents.tableAlign" header-align="center" label="操作">
            <template slot-scope="scope">
              <el-button
                v-if="isAuth('wodehaoyou', '修改') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">{{
                  contents.tableBtnFont == 1?'修改':'' }}</el-button>
              <el-button
                v-if="isAuth('wodehaoyou', '修改') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
              ...
              <el-button v-if="isAuth('wodehaoyou', '修改') && contents.tableBtnIcon == 0" type="primary" size="mini"
              ...
              <el-button
                v-if="isAuth('wodehaoyou', '删除') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">{{
                  contents.tableBtnFont == 1?'删除':'' }}</el-button>
              <el-button
                v-if="isAuth('wodehaoyou', '删除') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
              ...
              <el-button v-if="isAuth('wodehaoyou', '删除') && contents.tableBtnIcon == 0" type="danger" size="mini"
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      if (this.searchForm.tianjiashijianstart != '' && this.searchForm.tianjiashijianstart != undefined) {
        params['tianjiashijianstart'] = this.searchForm.tianjiashijianstart
      }
      if (this.searchForm.tianjiashijianend != '' && this.searchForm.tianjiashijianend != undefined) {
        params['tianjiashijianend'] = this.searchForm.tianjiashijianend
      }
      this.$http({
        url: "wodehaoyou/page",
        method: "get",
