  ...
  <div class="addEdit-block">
    <el-form class="detail-form-content" ref="ruleForm" :model="ruleForm" :rules="rules" label-width="80px"
      ...
      <el-row>
        ...
        <el-col :span="12">
          <el-form-item class="date" v-if="type != 'info'" label="添加时间" prop="tianjiashijian">
            <el-date-picker value-format="yyyy-MM-dd HH:mm:ss" v-model="ruleForm.tianjiashijian" type="datetime"
              placeholder="添加时间">
            </el-date-picker>
          </el-form-item>
          <div v-else>
            <el-form-item class="input" v-if="ruleForm.tianjiashijian" label="添加时间" prop="tianjiashijian">
              <el-input v-model="ruleForm.tianjiashijian" placeholder="添加时间" readonly></el-input>
            </el-form-item>
          </div>
        </el-col>
      </el-row>
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `wodehaoyou/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.wodehaoyouCrossAddOrUpdateFlag = false;
                  this.parent.search();
